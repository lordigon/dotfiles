...
main() {
  ...
  command -v git &>/dev/null || {
    print_error "Git must be installed in you machine. dotfiles installation stopped."
    exit 1
  }

  # Check that nc (netcat utility) is installed
  command -v nc &>/dev/null || {
    print_error "nc command must be installed in you machine. dotfiles installation stopped."
  }
